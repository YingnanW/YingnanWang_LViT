...
#     from datasets_3class import image_datasets,dataloders# 导入数据集
# else:
        ...
        for phase in ['train', 'val']:
            ...
            for images, labels in dataloders[phase]:               
                ...
                if phase == 'train':
                    ...
                    opt = model(images)
                    if args.dataset == 'LCI' or args.dataset == 'WLI':
                        opt_softmax = torch.nn.functional.softmax(opt, dim=1)  #三分类
                        _,preds = torch.max(opt_softmax,1)
                    else:
            ...
            if phase == 'val' and epoch_acc > best_acc:
                if not os.path.exists(f'/home/mch/our_model/LIVIT_3/2_best_model/{args.optim}-{args.model}-{args.dataset}-lr{args.lr}-momentum{args.momentum}-gamma{args.gamma}-wd{args.weight_decay}-epoch{args.epochs}/'):
                    os.makedirs(f'/home/mch/our_model/LIVIT_3/2_best_model/{args.optim}-{args.model}-{args.dataset}-lr{args.lr}-momentum{args.momentum}-gamma{args.gamma}-wd{args.weight_decay}-epoch{args.epochs}/')
                best_acc = epoch_acc
                best_model_wts = model.state_dict()
                torch.save(best_model_wts,f'/home/mch/our_model/LIVIT_3/2_best_model/{args.optim}-{args.model}-{args.dataset}-lr{args.lr}-momentum{args.momentum}-gamma{args.gamma}-wd{args.weight_decay}-epoch{args.epochs}/best_model.pkl')# 存储最好的模型
            if phase == 'train':
