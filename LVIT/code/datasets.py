...
data_transforms={
            'train':transforms.Compose([
                transforms.RandomChoice([
                    transforms.Compose([transforms.RandomHorizontalFlip(p=0.5),transforms.RandomVerticalFlip(p=0.5)]),          
                    transforms.RandomRotation(degrees=180),           # 
                    transforms.RandomAffine(degrees=0, translate=(0.2, 0.2), 
                                    scale=None, shear=None, interpolation=InterpolationMode.BILINEAR, fill=0), # 
...
image_datasets = {x: customData(txt_path=(f'/home/our_model/Gate_LIVIT/{args.dataset}'+'-' + x +B + '.txt'),
