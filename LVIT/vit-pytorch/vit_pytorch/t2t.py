...
#     def __init__(self, *, image_size, num_classes, dim, depth = None, heads = None, mlp_dim = None, pool = 'cls', channels = 3, dim_head = 64, dropout = 0., emb_dropout = 0., transformer = None, t2t_layers = ((31, 4), (15, 2))):
#         super().__init__()
...
#             output_image_size = conv_output_size(output_image_size, kernel_size, stride, (kernel_size-1)// 2)

#             layers.extend([
#                 RearrangeImage() if not is_first else nn.Identity(),
#                 # nn.Unfold(kernel_size = kernel_size, stride = stride, padding =stride// 2),
#                 #更改
#                 nn.Unfold(kernel_size = kernel_size, stride = stride, padding =(kernel_size-1)// 2),
...
def t2t_vit_14(pretrained=False, **kwargs):  # adopt performer for tokens to token
    ...
    model = T2T_ViT(tokens_type='performer',num_classes=2, embed_dim=384, depth=14, num_heads=6, mlp_ratio=3., **kwargs)
...
def t2t_vit_19(pretrained=False, **kwargs): # adopt performer for tokens to token
    ...
    model = T2T_ViT(tokens_type='performer',num_classes=2, embed_dim=448, depth=19, num_heads=7, mlp_ratio=3., **kwargs)
...
def t2t_vit_24(pretrained=False, **kwargs): # adopt performer for tokens to token
    ...
    model = T2T_ViT(tokens_type='performer',num_classes=2, embed_dim=512, depth=24, num_heads=8, mlp_ratio=3., **kwargs)
