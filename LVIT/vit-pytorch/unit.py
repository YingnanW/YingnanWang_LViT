...
def test_model(model, criterion, device,logger):
    ...
    num_epochs =1
    test_LOSS,test_ACC = [] ,[]
    model.load_state_dict(torch.load(f'/home/mch/vit-pytorch/2_best_model/{args.model}-{args.dataset}-lr{args.lr}-momentum{args.momentum}-gamma{args.gamma}-epoch{args.epochs}/best_model.pkl'))
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            print('{} Loss: {:.4f} ACC: {:.4f} SENS: {:.4f} SPEC: {:.4f} PPV: {:.4f} NPV: {:.4f}'.format('test', test_epoch_loss, test_epoch_acc,test_sens,test_spec,test_ppv,test_npv))
