...
class Bottleneck(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(3, 64, kernel_size=(1, 1), stride=(1, 1), bias=False)
        ...
        self.conv3 = nn.Conv2d(64, 64, kernel_size=(1, 1), stride=(1, 1), bias=False)
...
# print(backbone)
# backbone.load_state_dict(torch.load('/home/mch/vit-pytorch/out/Maxvit_LCIGA/-best_model.pkl'))
...
class OurModel(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Linear(512*k + 1000,2)
        self.fc1 = nn.Linear(512*k + 1000,256)
        self.fc2 = nn.Linear(256,2)
        self.relu = nn.ReLU(inplace=True)
        self.drop = nn.Dropout(p=0.2)
    def forward(self, x0, x_):
        y0 = self.backbone(x0)
        y_ = self.block(x_)# y_ [bs* 5, f]
        y_ = y_.view(-1, y_.size()[-1]*k)
        # y0 = y0.unsqueeze(1) # [bs, f] -> [bs, 1, f]
        y = torch.cat([y0, y_], dim=1)
...
def get_model(model_name):
    model = OurModel()
    return model
Maxvit_conv = get_model(args.net)