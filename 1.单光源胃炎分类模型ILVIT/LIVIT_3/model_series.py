...
def CSA(
    ...
    *,
    ...
    if dim_in == dim_out and not downsample:
        net =  CSA_Residual(net, dropout = dropout) #残差
...
class ILViT_series(nn.Module):
    def __init__(
        ...
        *,
        ...
        # print(dims)  # 输出: (96, 192, 384)
        dims = (dim_conv_stem, *dims)
        # print(dims)  # 输出: (64, 96, 192, 384))
        dim_pairs = tuple(zip(dims[:-1], dims[1:]))
        # dims[:-1] 将是 (64, 96, 192),dims[1:] 将是 (96,  192, 384),zip(dims[:-1], dims[1:]):
        # zip 函数将这两个元组“压缩”在一起，生成一对对的元素。
        #  具体来说，它将第一个元组的第 i 个元素和第二个元组的第 i 个元素配对。
        # print(dim_pairs)  # 输出: ((64, 96), (96, 192), (192, 384))
        ...
        for ind, ((layer_dim_in, layer_dim), layer_depth) in enumerate(zip(dim_pairs, depth)):
            for stage_ind in range(layer_depth):
                ...
                block1 = nn.Sequential(
                    CSA(
                        dim_in = stage_dim_in,#输入的维度
                        dim_out = layer_dim,#输出的维度
                        downsample = is_first,
                    ...
                    Residual(layer_dim, LightMSA(dim = layer_dim, dim_head = dim_head, dropout = dropout, window_size = w, sr_ratio = sr_ratio)),
                    Rearrange('b x y w1 w2 d -> b d (x w1) (y w2)'),
                    Residual(layer_dim, IRFFN(in_channels = layer_dim)),
                ...
                self.layers.append(nn.ModuleList([block1,grid2]))
    ...
    def forward(self, x):
        x = self.conv_stem(x)
        for block1, grid2 in self.layers:
                b, c, h, w = x.shape
...
model.to('cuda:1')
# 创建一个与训练集输入形状相同的示例输入
input = torch.randn(1, 3, 224, 224).to('cuda:1')
